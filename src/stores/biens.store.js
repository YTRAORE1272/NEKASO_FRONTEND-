...
export const useBiensStore = defineStore('biens', () => {
  ...
  async function creer(donnees) {
    ...
    try {
      ...
      console.log("Mock API - POST /biens avec FormData:", formData)
  ...
  async function modifier(id, donnees) {
    ...
    try {
      ...
      const formData = buildFormData(donnees)
      console.log(`Mock API - PUT /biens/${id} avec FormData:`, formData)
  ...
  async function archiver(id) {
    try {
      console.log(`Mock API - PATCH /biens/${id}/archiver`)
  ...
  async function supprimerPhoto(bienId, photoId) {
    try {
      console.log(`Mock API - DELETE /biens/${bienId}/photos/${photoId}`)
      await new Promise(resolve => setTimeout(resolve, 300))
  ...
  return {
    ...
    charger, creer, modifier, archiver, supprimerPhoto,
  }
})